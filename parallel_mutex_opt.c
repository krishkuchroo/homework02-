...
// Per-bucket mutexes for fine-grained locking
...
void insert(int key, int val) {
  ...
  // Lock only the specific bucket we're inserting into
...
bucket_entry * retrieve(int key) {
  ...
  // We need a read lock to prevent the linked list from being modified
  // while we're traversing it. However, multiple readers can access
  // the same bucket simultaneously if we use a reader-writer lock.
  // For simplicity with mutexes, we still lock, but this could be
  // optimized with pthread_rwlock_t for true read parallelization.
  pthread_mutex_lock(&bucket_mutexes[i]);
